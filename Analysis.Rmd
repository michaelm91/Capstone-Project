...
alc_data <- drivers %>%
  mutate(
    alc = driver_substance_abuse_alcohol_contributed | driver_substance_abuse_alcohol_present,
    injury_severity = factor(injury_severity, ordered_injury)
  ) %>%
  select(report_number,alc, injury_severity) %>%
  group_by(report_number) %>%
  summarise(alc = any(alc), injury_severity = ordered_injury[max(as.integer(injury_severity))]) %>%
  mutate(injury_severity = factor(injury_severity,ordered_injury))

temp <- chisq.test(as.factor(alc_data$alc), as.factor(alc_data$injury_severity))
temp %>%
  broom::tidy() %>%
  knitr::kable()
```
...
```{r alctable}
contingency_table(alc_data, alc, injury_severity)
```
